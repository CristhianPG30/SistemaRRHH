...
if (isset($_GET['toggle_id'])) {
    ...
    // Selecciona el colaborador único asociado a la persona
    ...
    if ($result->num_rows == 1) {
        ...
        $message = 'Error: No se puede determinar el colaborador a actualizar (verifica que no hay duplicados).';
        $message_type = 'danger';
    }
    $stmt->close();
}
...
if (isset($_POST['delete_id']) && isset($_POST['delete_name'])) {
    ...
    try {
        // Buscar el/los idColaborador de la persona
        $colabs = [];
        ...
        if (count($colabs) > 0) {
            ...
            // Actualiza id_jefe_fk a 1 (admin) en vez de NULL (por restricción de tu modelo)
            $conn->query("UPDATE colaborador SET id_jefe_fk = 1 WHERE id_jefe_fk IN ($colabs_imploded) AND id_jefe_fk <> 1");
            // Borra relaciones en jerarquia (ambos campos)
            ...
            // Borra de todas las tablas hijas
        ...
        // Borra dependencias directas de persona
...
// --- Lógica de visualización e identificación ---
...
<head>
    ...
    <style>
        ...
        .table-header th { border-color: #4e73df; }
...
<body>
                                ...
                                <?php while ($row = $result_personas->fetch_assoc()): ?>
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            ...
                                            <button type="button" class="btn btn-light btn-sm btn-action" title="Eliminar" onclick="confirmDelete(<?php echo $row['idPersona']; ?>, '<?php echo htmlspecialchars($row['Nombre'] . " " . $row['Apellido1'] . " " . $row['Apellido2']); ?>')">
    ...
    <script>
        ...
        // Modal de confirmación con nombre personalizado
        function confirmDelete(id, nombre) {
            document.getElementById('delete_id_input').value = id;
            document.getElementById('delete_name_input').value = nombre;
            document.getElementById('deleteModalText').innerHTML =
                "¿Estás seguro de que deseas eliminar a <b>" + nombre + "</b>?<br> <span class='text-danger fw-bold'>Esta acción eliminará TODOS sus datos del sistema.</span>";
