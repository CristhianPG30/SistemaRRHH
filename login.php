<?php
// 1. LÓGICA DE BACKEND (SEGURA Y SIN CAMBIOS)
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($username) || empty($password)) {
        ...
        $sql = "SELECT u.idUsuario, u.username, u.password, u.id_rol_fk, u.id_persona_fk, c.idColaborador, c.activo FROM usuario u LEFT JOIN colaborador c ON u.id_persona_fk = c.id_persona_fk WHERE BINARY u.username = ?";
            ...
            if ($result->num_rows === 1) {
                ...
                if (password_verify($password, $user['password'])) {
                    if ($user['activo'] == 1 || is_null($user['activo'])) {
                        ...
                        exit();
                    } else { $errorMessage = "Tu cuenta ha sido desactivada."; }
                } else { $errorMessage = "Usuario o contraseña incorrectos."; }
            } else { $errorMessage = "Usuario o contraseña incorrectos."; }
            $stmt->close();
        } else { $errorMessage = "Error interno del sistema."; }
    }
    $conn->close();
}
?>
