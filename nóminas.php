...
$anio_seleccionado = intval($_POST['anio'] ?? $_GET['anio'] ?? date('Y'));
$mes_seleccionado = intval($_POST['mes'] ?? $_GET['mes'] ?? date('n'));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['eliminar_planilla'])) {
    ...
    try {
        ...
        $conn->rollback();
        $mensaje = '<div class="alert alert-danger alert-dismissible fade show">Error al eliminar la planilla.</div>';
    }
}
...
function calcularDeduccionesDeLey($salario_bruto, $conn) {
    ...
    $result = $conn->query("SELECT Descripcion FROM tipo_deduccion_cat");
    if ($result) { while ($row = $result->fetch_assoc()) { @list($nombre, $porcentaje) = explode(':', $row['Descripcion']); if (is_numeric(trim($porcentaje))) { $monto = $salario_bruto * (floatval(trim($porcentaje)) / 100); $deducciones['total'] += $monto; $deducciones['detalles'][] = ['descripcion' => trim($nombre), 'monto' => $monto, 'porcentaje' => $porcentaje]; } } }
    return $deducciones;
}
...
function obtenerPlanilla($anio, $mes, $conn) {
    ...
    while ($colaborador = $result_colaboradores->fetch_assoc()) {
        ...
        $stmt_asist->bind_param("iii", $idPersona, $mes, $anio); $stmt_asist->execute();
        ...
        $stmt_perm->bind_param("i", $idColaborador); $stmt_perm->execute();
        ...
        $stmt_perm->close();
        $permisos_del_mes = array_unique($permisos_del_mes);

        $dias_ausencia = 0;
        foreach($dias_laborales_del_mes as $dia_laboral){
            if(!in_array($dia_laboral, $asistencias_del_mes) && !in_array($dia_laboral, $permisos_del_mes)){
                $dias_ausencia++;
            }
        }
        $rebajo_ausencias = $dias_ausencia * $salario_diario;
        ...
        $salario_bruto_ajustado = ($salario_base - $rebajo_ausencias) + $pago_horas_extra;
        $id_categoria = determinarCategoriaSalarial($salario_bruto_ajustado, $categorias_salariales);
        $deducciones = calcularDeduccionesDeLey($salario_bruto_ajustado, $conn);
        $impuesto_renta = calcularImpuestoRenta($salario_bruto_ajustado - $deducciones['total']);
        $total_deducciones_final = $deducciones['total'] + $impuesto_renta;
        $salario_neto = $salario_bruto_ajustado - $total_deducciones_final;

        $planilla[] = array_merge($colaborador, [
            'id_categoria_salarial_fk' => $id_categoria, 'Salario_bruto_ajustado' => $salario_bruto_ajustado,
            'Total_horas_extra' => $total_horas_extra, 'Pago_horas_extra' => $pago_horas_extra,
            'Rebajo_ausencias' => $rebajo_ausencias, 'Dias_ausencia' => $dias_ausencia,
            'Deducciones_detalles' => $deducciones['detalles'], 'Impuesto_renta' => $impuesto_renta,
            'Total_deducciones' => $total_deducciones_final, 'Salario_neto' => $salario_neto
        ]);
    }
    return $planilla;
}
function guardarPlanilla($planilla, $anio, $mes, $conn) {
    // ...
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['generar_planilla'])) {
    if ($puede_generar) {
        guardarPlanilla($planilla_previsualizada, $anio_seleccionado, $mes_seleccionado, $conn);
            ...
            <div class="card-body">
                <form method="POST" class="mb-4">
                    <div class="row align-items-end">
                        <div class="col-md-4 mb-3"><label class="form-label">Mes:</label><select name="mes" class="form-select"><?php foreach ($meses_espanol as $num => $nombre) echo "<option value='$num' ".($num==$mes_seleccionado?'selected':'').">$nombre</option>"; ?></select></div>
                        <div class="col-md-4 mb-3"><label class="form-label">Año:</label><select name="anio" class="form-select"><?php for ($y=date('Y')-2; $y<=date('Y'); $y++) echo "<option value='$y' ".($y==$anio_seleccionado?'selected':'').">$y</option>"; ?></select></div>
                        <div class="col-md-4 mb-3"><button type="submit" name="filtrar" class="btn btn-secondary w-100"><i class="bi bi-search"></i> Previsualizar</button></div>
                    </div>
                </form>
                ...
                <div class="text-end mt-3">
                    <form method="POST">
                        <input type="hidden" name="mes" value="<?= date('n') ?>"><input type="hidden" name="anio" value="<?= date('Y') ?>">
                ...
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            ...
                            <?php endforeach; else: ?>
                            <tr><td colspan="2" class="text-center text-muted">No hay planillas en el historial.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
    ...
    <div class="modal fade" id="detalleModal<?= $col['idColaborador'] ?>" tabindex="-1">
        <div class="modal-dialog modal-lg"><div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <p class="text-danger"><strong>Rebajo por Ausencias (<?= $col['Dias_ausencia'] ?> días):</strong> - ₡<?= number_format($col['Rebajo_ausencias'], 2) ?></p>
                ...
                <hr><p><strong>Salario Bruto Ajustado:</strong> ₡<?= number_format($col['Salario_bruto_ajustado'], 2) ?></p><hr>
                <h6>Deducciones de Ley</h6>
                <ul><?php foreach ($col['Deducciones_detalles'] as $deduccion): ?><li><?= htmlspecialchars($deduccion['descripcion']) ?> (<?= $deduccion['porcentaje'] ?>%): - ₡<?= number_format($deduccion['monto'], 2) ?></li><?php endforeach; ?></ul>
                <p><strong>Impuesto sobre la Renta:</strong> - ₡<?= number_format($col['Impuesto_renta'], 2) ?></p>
