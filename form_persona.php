<?php
// --- INICIO DE LÓGICA DE ACCIONES (ELIMINAR / AJAX) ---

// Iniciar sesión y conectar a la BD una sola vez al principio
...
// --- NUEVO BLOQUE PARA MANEJAR LA ELIMINACIÓN (INACTIVACIÓN) ---
if (isset($_GET['action']) && $_GET['action'] == 'delete') {
    // Verificación de Seguridad: Solo Administrador (rol 1) puede inactivar
    if (!isset($_SESSION['username']) || !in_array($_SESSION['rol'], [1])) {
    ...
    if (isset($_GET['id']) && !empty($_GET['id'])) {
        ...
        // Usamos UPDATE para cambiar el estado en 'colaborador' a inactivo (0)
        $sql = "UPDATE colaborador SET activo = 0 WHERE id_persona_fk = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $idPersona);

        if ($stmt->execute()) {
            $_SESSION['flash_message'] = ['type' => 'success', 'message' => 'La persona ha sido marcada como inactiva.'];
        } else {
            $_SESSION['flash_message'] = ['type' => 'danger', 'message' => 'Error al actualizar el estado de la persona.'];
        }
        $stmt->close();
    } else {
        $_SESSION['flash_message'] = ['type' => 'danger', 'message' => 'No se especificó un ID válido.'];
    }

    $conn->close();
    header('Location: personas.php'); // Redirigir de vuelta a la lista
    exit; // Detener la ejecución del script
}

// --- LÓGICA AJAX PARA VERIFICAR CÉDULA ---
if (isset($_POST['action']) && $_POST['action'] == 'check_cedula') {
    header('Content-Type: application/json');
    $cedula = isset($_POST['cedula']) ? preg_replace('/[^a-zA-Z0-9]/', '', $_POST['cedula']) : '';
    ...
    $sql = "SELECT idPersona FROM persona WHERE Cedula = ?";
    $params = [$cedula];
    $types = "s";
    if ($idPersona > 0) {
        $sql .= " AND idPersona != ?";
...
// --- INICIO DE LÓGICA NORMAL DE LA PÁGINA (MOSTRAR FORMULARIO Y PROCESAR POST) ---

// Acceso para Administrador (1) y Recursos Humanos (4) para ver el formulario
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !(isset($_POST['action']) && $_POST['action'] == 'check_cedula')) {
    ...
    // (resto de la recolección de datos que ya tenías...)
    ...
    $cedula_check_sql = "SELECT idPersona FROM persona WHERE Cedula = ? ";
    if ($is_edit_mode) {
        $cedula_check_sql .= "AND idPersona != ?";
        $stmt_ced = $conn->prepare($cedula_check_sql);
        $stmt_ced->bind_param("si", $cedula_limpia, $idPersona);
    } else {
        $stmt_ced = $conn->prepare($cedula_check_sql);
        $stmt_ced->bind_param("s", $cedula_limpia);
    }
    $stmt_ced->execute();
    $cedula_exists = $stmt_ced->get_result()->num_rows > 0;
    $stmt_ced->close();

    if ($cedula_exists) {
        $errors[] = "Ya existe una persona registrada con esta cédula o DIMEX.";
    }
    ...
    if (!empty($errors)) {
        ...
        try {
            // (Toda tu lógica de transacciones que ya tenías para guardar/actualizar...)
            // Dirección
...
<body>
                ...
                <div class="card p-4">
                    ...
                    <div class="form-step active" data-step-content="1">
                        ...
                        <div class="row">
                                ...
                                <select class="form-select" id="nacionalidad" name="nacionalidad" required>
                                    ...
                                    if ($nacionalidades_db && $nacionalidades_db->num_rows > 0) {
                                        ...
                                        while ($nac = $nacionalidades_db->fetch_assoc()):
                                            ...
                                            $tipo_doc = strpos($descripcion_lower, 'extranjero residente') !== false ? 'DIMEX' : 'Cedula';
                                    ?>
                                            <option value="<?= $nac['idNacionalidad']; ?>" data-tipo-doc="<?= $tipo_doc; ?>" <?= ($nacionalidad_id == $nac['idNacionalidad']) ? 'selected' : ''; ?>>
                            ...
                            <div class="col-md-6 mb-3"><label for="cantidad_hijos" class="form-label required">Cantidad de Hijos</label><input type="number" class="form-control" name="cantidad_hijos" value="<?= htmlspecialchars($cantidad_hijos); ?>" min="0" max="25" required></div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        async function validateStep(stepIndex) {
            ...
            const inputsToValidate = currentFormStep.querySelectorAll('input[required], select[required], textarea[required]');
            ...
            for (const input of inputsToValidate) {
                ...
                if (!valueToTest && input.required) {
                    ...
                    if (input.name === 'cantidad_hijos' && input.hasAttribute('max') && parseInt(valueToTest) > parseInt(input.max)) {
                        hasError = true;
                        errorMessage = 'La cantidad de hijos no puede ser mayor a 25.';
                    }
            ...
            if (stepIndex === 1 && cedulaInput.value.trim() !== '' && !cedulaInput.classList.contains('is-invalid') && isValid) {
                const formData = new FormData();
                formData.append('action', 'check_cedula');
                formData.append('cedula', cedulaInput.value);
                formData.append('idPersona', document.querySelector('input[name="idPersona"]').value);

                try {
                    const response = await fetch('form_persona.php', {
                        method: 'POST',
                        body: formData
                    });
                    ...
                    if (data.exists) {
                        isValid = false;
                        const errorMessage = "Esta cédula o DIMEX ya se encuentra registrada.";
                        if (!errorMessages.includes(errorMessage)) {
                            errorMessages.push(errorMessage);
                        }
                    ...
                    errorMessages.push('No se pudo validar la identificación. Inténtelo de nuevo.');
                    isValid = false;
                }
            }
        ...
        document.querySelectorAll('[data-nav]').forEach(button => {
            button.addEventListener('click', async () => {
                ...
                if (direction === 1 && !(await validateStep(currentStep))) {
                    return;
                }
        ...
        stepper.querySelectorAll('.step').forEach(step_el => {
            step_el.addEventListener('click', async () => {
        ...
        telefonoInput.pattern = '^[245678]\\d{3}-?\\d{4}$';
